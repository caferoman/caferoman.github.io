...
permalink: wiki_build_scripts.html
folder: wiki
---
...
The wiki project has 5 build scripts and a script that runs them all. These scripts will require a bit of detail to configure. Every team member who is publishing on the project should set up their folder structure in the way described here.
...
For the wikis project, you'll see a series of build scripts for each project. There are 5 build scripts, described in the following sections. Note that you really only need to run the last one, e.g., wiki_all.sh, because it runs all of the build scripts. But you have to make sure each script is correctly configured so that they all build successfully.

{% include tip.html content="In the descriptions of the build scripts, \"wiki\" is used as the sample project. Substitute in whatever your real project name is." %}

### wiki_1_multiserve_pdf.sh
...
jekyll serve --detach --config configs/wiki/config_writers.yml,configs/wiki/config_writers_pdf.yml
echo "done"

echo "Building PDF-friendly HTML site for Mydoc Designers ..."
jekyll serve --detach --config configs/wiki/config_designers.yml,configs/wiki/config_designers_pdf.yml
echo "done"

echo "All done serving up the PDF-friendly sites. Now let's generate the PDF files from these sites."
echo "Now run . wiki_2_multibuild_pdf.sh"
```
...
Each of your configuration files needs to have a destination like this: `../doc_outputs/wiki/adtruth-java`. That is, the project should build in the doc_outputs folder, in a subfolder that matches the project name.
...
prince --javascript --input-list=../doc_outputs/wiki/writers-pdf/prince-file-list.txt -o wiki/files/wiki_writers_pdf.pdf;
echo "done"

# Doc Designers
echo "Building Mydoc Designers PDF ..."
prince --javascript --input-list=../doc_outputs/wiki/designers-pdf/prince-file-list.txt -o wiki/files/wiki_designers_pdf.pdf;
echo "done"

echo "All done building the PDFs!"
echo "Now build the web outputs: . wiki_3_multibuild_web.sh"
```
...
The Prince build script puts the output PDF into the wiki/wiki/files directory. Now you can reference the PDF file in your HTML site. For example, on the homepage you can allow people to download a PDF of the content at files/adtruth_dotnet_pdf.pdf.

### wiki_3_multibuild_web.sh
...
echo "Now push the builds to the server with . wiki_4_publish.sh"
```
...
scp -r -vrC ../wiki_outputs/doc-writers yourusername@yourdomain:/var/www/html/yourpublishingdirectory
...
Also, in case you haven't set up the SSH publishing without a password, see [Getting around the password prompts in SCP][wiki_no_password_prompts_scp]. Otherwise the script will stop and ping you to enter your password for each directory it transfers.
...
The doc_outputs/wiki/designers contains the site output from wiki, so when you push content from this folder into Github, you're actually pushing the HTML site output into Github, not the wiki source files.
...
After you've configured all the scripts, you can run them all by running `. wiki_all.sh`. You might want to run this script at lunchtime, since it may take about 10 to 20 minutes to completely build the scripts. But note that since everything is now automated, you don't have to do anything at all after executing the script. After the script finishes, everything is published and in the right location.
