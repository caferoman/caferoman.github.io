...
permalink: wiki_generating_pdfs.html
...
<a target="\_blank" class="noCrossRef" href="{{ "pdf/wiki.pdf"}}"><button type="button" class="btn btn-default" aria-label="Left Align"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> PDF Download</button></a>
...
You can see the details of the script in these files in the theme's root directory. For example, open pdf-wiki.sh. It contains the following:
...
bundle exec jekyll serve --detach --config _config.yml,pdfconfigs/config_wiki_pdf.yml;
echo "done";

echo "Building the PDF ...";
prince --javascript --input-list=_site/pdfconfigs/prince-list.txt -o pdf/wiki.pdf;
...
The PDF configuration file will build on the settings in the regular configuration file but will some additional fields. Here's the configuration file for the wiki product within this theme. This configuration file is located in the pdfconfigs folder.
...
output: pdf
product: wiki
print_title: Jekyll theme for documentation — wiki product
...
These pages should appear in your sidebar YML file (in this product, wiki_sidebar.yml):
...
{% include note.html content="If you have any files that you do not want to appear in the PDF, add <code>output: web</code> (rather than <code>output: pdf</code>) in the list of attributes in your sidebar. The prince-list.txt file that loops through the wiki_sidebar.yml file to grab the URLs of each page that should appear in the PDF will skip over any items that do not list <code>output: pdf</code> in the item attributes. For example, you might not want your tag archives to appear in the PDF, but you probably will want to list them in the online help navigation." %}
...
Duplicate the pdf-wiki.sh file in the root directory and customize it for your specific configuration files.
...
echo "Building PDF-friendly HTML site for Mydoc ...";
jekyll serve --detach --config _config.yml,pdfconfigs/config_wiki_pdf.yml;
echo "done";
...
```html
<a target="_blank" rel="noopener" class="noCrossRef" href="/pdf/wiki.pdf"><button type="button" class="btn btn-default" aria-label="Left Align"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> PDF Download</button></a>
```
...
For more detail about using `unless` in conditional logic, see [Conditional logic][wiki_conditional_logic]. What this code means is "run this code unless this value is the case."
