...
You can change the button color by changing the class on the button from `btn-info` to one of the other button classes bootstrap provides. See [Labels][wiki_labels] for more options on button class names.
...
See [WebStorm Text Editor][wiki_webstorm_text_editor] for tips on creating file templates in WebStorm.
